function item_email_UE(type)
{
	if(type=="create")
	{
		try
		{
			...
			//Check to see if transaction is on payment hold
			var paymentEventResult = order.getFieldValue("paymenteventresult");
					...
					if(paymentMethod=="9")
		              {
		                var emailMergerPayPal = nlapiCreateEmailMerger(3);
							...
							var emailBody = mergeResult.getBody();
		                nlapiSendEmail(itemDetails.custitem_email_from,customer,mergeResult.getSubject(),emailBody,null,["peni@migdalohrusa.org","atara@migdalohrusa.org"],records,null,true);
		              }else{
		                nlapiSendEmail(itemDetails.custitem_email_from,customer,mergeResult.getSubject(),emailBody,null,["peni@migdalohrusa.org","atara@migdalohrusa.org"],records,null,true);
		              }
