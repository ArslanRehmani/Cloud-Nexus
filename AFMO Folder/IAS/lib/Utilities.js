...
define(
	...
	function(search, runtime, format, email, error, record) {
		 ...
		 */
		Util.inArray = function(stValue, arrValue) {
		 ...
		 */
		Util.forceInt = function(stValue) {
		 ...
		 */
		Util.removeDuplicate = function(arrValue) {
		 ...
		 */
		Util.replaceCharAt = function(stValue, intPos, stReplacement) {
		 ...
		 */
		Util.insertStringAt = function(stValue, intPos, stInsert) {
			return (
				[
		 ...
		 */
		Util.formatCurrency = function(flValue, stCurrencySymbol, intDecimalPrecision) {
		 ...
		 * @returns {String} - formatted value
		 */
		Util.formatPercent = function(flValue, stPercentSymbol, intDecimalPrecision) {
			var flAmount = flValue;
		 ...
		 */
		Util.roundDecimalAmount = function(flDecimalNumber, intDecimalPlace) {
		 ...
		 */
		Util.getTimeBetween = function(dtStartDate, dtEndDate, stTime) {
		 ...
		 */
		Util.sanitizeFilename = function(stFileName) {
		 ...
		 */
		Util.toItemInternalId = function(stRecordType) {
			if (!stRecordType) {
		 ...
		 */
		Util.isClosedDatePostingPeriod = function(stDate) {
			var bIsClosed = true;
		 ...
		 * @returns {Boolean} bIsClosed - returns true if posting period is closed; otherwise returns false
		 */
		Util.isClosedPostingPeriod = function(stPeriodName) {
			var bIsClosed = true;
		 ...
		 */
		Util.getItemPrice = function(stItemId, stPriceLevel) {
			if (stPriceLevel == '1') {
		 ...
		 */
		Util.search = function(stRecordType, stSearchId, arrSearchFilter, arrSearchColumn, stSearchType) {
			...
			if (stSearchId != null) {
				...
				if (arrSearchFilter != null) {
					if (arrSearchFilter[0] instanceof Array || (typeof arrSearchFilter[0] == 'string')) {
						var filterExpression = objSavedSearch.filterExpression;
		 ...
		 */
		Util.sendEmailBasedOnRole = function(stRoleId, stAuthor, stCc, stBcc, stSubject, stBody, objFileAttachments, objRelatedRecords, bIsPromise) {
			var arrFilters = [];
			...
			var arrSearchResults = Util.search('employee', null, arrFilters, arrColumns);
			if (arrSearchResults) {
				var arrEmpIds = []
		 ...
		 */
		Util.searchArray = function(nameKey, value, myArray) {
		 ...
		 * @param char
		 */
		Util.addTrailingChar = function(text, size, char) {
			var s = '' + text + '';
			while (s.length < size)
				s = s + char;
			return s;
		 ...
		 * @param char
		 */
		Util.addLeadingChar = function(text, size, char) {
			var s = '' + text + '';
			while (s.length < size)
				s = char + s;
			return s;
		 ...
		 * 
		 */
		Util.overlayErrorMsg = function(errorObj) {
			if (errorObj) {
		 ...
		 * 
		 */
		Util.loadEnvironmentSettings = function(stConfigName) {
			return Util.loadEnvironmentSettings_('customrecord_p_envi_settings', 'custrecord_p_envi_settings_config', stConfigName);
		}
		 ...
		 * 
		 */
		Util.loadEnvironmentSettings_ = function(stEnviSettingRecordType, stEnviSettingRecordFieldId, stConfigName) {
			...
			objFileSearch.run().each(function(result) {
				...
				for (var i = 0; i < arrLines.length; i++) {
					...
					Util.defineEnumVariables(objEnum, stProp, stValue);
				}
				return true;
			});
			return objEnum;
		 ...
		 * 
		 */
		Util.defineEnumVariables = function(objEnum, stProp, stValue) {
